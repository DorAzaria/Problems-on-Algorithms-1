...
/*
תיאור הבעיה:
3 ציידים נלחמים זה בזה, לצייד הראשון יש 100% פגיעה , לשני 80% ולשלישי 50%, סדר השחקנים נקבע באופן רנדומלי
בהתאם לעדיפות שהציידים מקבלים בשלב הראשוןהם יורים אחד בשני עד שרק אחד נשאר בחיים, כל צייד יכול ןירות
באחד מהשניים האחרים לפי בחירתו או לירות באוויר.

פיתרון:
בשלב הראשון: קובעים את  סדר הציי
דים בצורה רנדומלית, פונקצית עזר : ולאחר מכן נכסה את המקרים בהתאם לכל סדר ציידים אפשרי
נתחיל את המשחק
 */
public class shots3 {
    public static void duel3d(int num){
        ...
        for (int i = 1; i <= num; i++) {
            int[] q=getQueuq3();
            int first = q[1];
//כל מקרה מדובר במשחק שבו שחקן אחד כבר נהרג
            switch (first) {
